...
KAFKA_MAX_MESSAGES = 10  # Number of messages to calculate estimated_offset.
KAFKA_ROUND_TRIP = .0131  # Estimated LAN Round Trip as 13.1 ms.
    ...
    def poll_messages(self, handle_function, current_keys):
        ...
        try:
            ...
            if key in self.message_ages:
                if len(self.message_ages[key]) < KAFKA_MAX_MESSAGES:
                    self.message_ages[key].append(message_age)
                    # Once we have enough samples, compute estimated offset
                    if len(self.message_ages[key]) == KAFKA_MAX_MESSAGES:
                        avg_age = sum(self.message_ages[key]) / KAFKA_MAX_MESSAGES
                        self.estimated_offset[key] = avg_age - KAFKA_ROUND_TRIP
