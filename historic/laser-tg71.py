from kafka_messaging24 import KafkaMessenger, KafkaMessageHandler, KafkaMapReader
import string
import pygame
import random
import time
...
SHAKE_INTERVAL = 3000  # 3 seconds per set
SHAKE_DURATION = 200   # how long each shake lasts
SHAKE_FREQUENCY = 10  # time between each shake step
SHAKE_ANGLE = 25        # max tilt angle
...
while running:
    ...
    if game_state == GAME_MENU:
        ...
        is_shaking = shake_elapsed < SHAKE_DURATION and (shake_elapsed // SHAKE_FREQUENCY) % 2 == 0

        # WASD
        if current_shake_set == "WASD": # and is_shaking
            t = (pygame.time.get_ticks() - last_shake_start) / SHAKE_DURATION  # from 0 to 1
            angle = math.sin(t * 4) * SHAKE_ANGLE  # 2 full oscillations
            print (angle)
        ...
        if pressed[pygame.K_F5]: game_state = GAME_PLAYING
