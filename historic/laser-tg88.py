...
while running:
    ...
    if game_state == GAME_MENU:
        ...
        if keys != prev_keys:
            messenger.send_wasd_update((game.player.rect.x, game.player.rect.y), keys)
            prev_keys = keys
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                running = False
                ...
                if len(keys) == 0: messenger.send_wasd_update((game.player.rect.x, game.player.rect.y), keys)
            else:
        ...
        # Health check
        if game.player.health <= 0: running = False
        for enemy in game.local_enemies[:]:
            if enemy.health <= 0:
                pygame.time.set_timer(enemy.shoot_timer, 0)
                game.local_enemies.remove(enemy)
            else:
                enemy.move(maze.bricks)
                enemy.draw(screen)
        for remote_id, enemy in list(game.remote_enemies.items()):  # Shalow copy because we're removing items while iterating them.
            if enemy.health <= 0:
                del game.remote_enemies[remote_id]
            else:
                enemy.move(maze.bricks)
                enemy.draw(screen)
        ...
        for bullet in game.bullets:
            ...
            if any(bullet.rect.colliderect(brick) for brick in maze.bricks):
            ...
            elif bullet.rect.colliderect(game.player.rect):
                game.player.hit()
                game.bullets.remove(bullet)
            else:
                ...
                for enemy in game.local_enemies:
                    if bullet.rect.colliderect(enemy.rect):
                        enemy.hit()
                        remove_bullet = True
                for enemy in game.remote_enemies.values():
                    if bullet.rect.colliderect(enemy.rect):
                        remove_bullet = True
                if remove_bullet: game.bullets.remove(bullet)
