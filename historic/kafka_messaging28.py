...
class KafkaMessenger:
    ...
    def send_waiting_2_join(self):
        payload = {}
        self._send("waiting_2_join", payload)
...
class KafkaMapReader:
    def __init__(self, player_id, bootstrap_servers="localhost:9092", topic=f"game-events"):
        ...
        self.map = None
        self.last_activity_age = 9999

    def poll_map_or_last_activity_age(self):
        ...
        if key == 'map':
            self.map = value
            print("Map got at {:.3f}s".format(time.time()-self.init_time))
        else:
            type = value.get("type")
            if type != "warmup":
                age = time.time() - value.get("timestamp")
                if age < self.last_activity_age:
                    self.last_activity_age = age
