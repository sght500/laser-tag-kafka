...
SHAKE_DURATION = 600   # how long each shake lasts
SHAKE_FREQUENCY = 100  # time between each shake step
SHAKE_ANGLE = 5        # max tilt angle

last_shake_start = pygame.time.get_ticks()
current_shake_set = "WASD"  # or "IJKL"
shake_phase = 0  # 0, 1, 2... to oscillate
...
while running:
    ...
    if game_state == GAME_MENU:
        ...
        if current_shake_set == "WASD" and is_shaking:
            angle = (-1) ** shake_phase * SHAKE_ANGLE
            shake_phase += 1
            blit_shaking_image(screen, WASD_surf, WASD_rect, angle)
            screen.blit(instr1_surf, instr1_rect)  # static text
        else:
            screen.blit(WASD_surf, WASD_rect)
            screen.blit(instr1_surf, instr1_rect)
