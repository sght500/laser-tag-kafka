from kafka_messaging10 import KafkaMessenger, KafkaMessageHandler, KafkaMapManager
import string
import pygame
import random
import time
...
# Player Class
class Player:
    def __init__(self, x, y, color, messenger=None):
...
def handle_kafka_message(sender_id, data):
    ...
    if msg_type == "wasd-update":
        ...
        if sender_id in remote_enemies:
            print (f"{sender_id} found in remote_enemies")
        ...
        # Add the bullet to your bullets list
...
if map:
    if last_activity:
        ...
        if current_time - last_activity < 600:  # 600 seconds = 10 minutes
            reuse_map = True
    else:
        reuse_map = True
...
while running:
    ...
    for enemy in enemies[:]:
        if enemy.health <= 0:
            ...
            enemy.draw(screen)
    for remote_id, enemy in list(remote_enemies.items()):
        if enemy.health <= 0:
