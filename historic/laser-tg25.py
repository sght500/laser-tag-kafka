    ...
    def move(self, keys, bricks):
        ...
        if self.processing_buffer:
            ...
            if buffer_time >= DG_BUFFER_TIME:
                ...
                game_bullets.append(self.process_buffer(NO_KEY))
...
def send_wasd_update(producer, player_id, position, keys):
    ...
    producer.produce("game-events", key=player_id, value=json.dumps(message).encode("utf-8"))
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN and event.key in [pygame.K_i, pygame.K_j, pygame.K_k, pygame.K_l]:
            ...
            if bullet: game_bullets.append(bullet)
