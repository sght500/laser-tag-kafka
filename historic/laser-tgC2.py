from kafka_messaging32 import KafkaMessenger, KafkaMessageHandler
import string
import pygame
import random
import time
...
MAX_ENEMY_TIMER = 325  # How often the local enemies shoot, the most?
...
class Player:
    ...
    def hit(self):
        ...
        if self.health > 0:
            play_random(player_hit_sounds)
        else:
            scream_channel.play(player_death_sound)
...
class Enemy:
    ...
    def hit(self):
        self.health -= BULLET_HURT_SPEED
        play_random(local_enemy_hit_sounds)

    def shoot(self, direction):
        pygame.time.set_timer(self.shoot_timer, random.randint(MIN_ENEMY_TIMER, MAX_ENEMY_TIMER))
        return create_bullet(self.rect, direction)
...
class RemoteEnemy:
    ...
    def hit(self):
        self.health -= BULLET_HURT_SPEED
        play_random(remote_enemy_hit_sounds)
...
class Game:
    ...
    def health_check_enemies(self, screen):
        """
        Check the health of the local enemies. Move them and draw them.
        Don't check health on remote enemies. Only Move them and draw them.
        """
...
def generate_map_id(data):
    trimmed = data[1:-1]
    digest = hashlib.sha1(bytes(trimmed)).hexdigest()
    return digest[:6].upper()  # 6-character hash

map_id_ww = {}
map_id_ex = {}
# Handling Kafka Messages in the Menu Loop
def get_map_from_idle(sender_id, data):
    global map, maze, status_start_time, menu_start_time
    msg_type = data.get("type")
    if msg_type in ["waiting-2-join", "wasd-update"]:
        map = data.get("map")
        b = [sum(x) for x in zip(*map)]
        map_id = generate_map_id(b)        
        if map_id != map_id_ww.get(sender_id):
            map_id_ww[sender_id] = map_id
            print(f"[{sender_id}] {msg_type} {map_id}")
    elif msg_type == "map-expired":
        b = [sum(x) for x in zip(*map)] if map else [15, 0, 15]
        map_id = generate_map_id(b)
        if map_id != map_id_ex.get(sender_id):
            map_id_ex[sender_id] = map_id
            print(f"[{sender_id}] {msg_type} {map_id}")
        map = None
        maze = None
        menu_start_time = time.time() + random.uniform(5, 11)  # Discourage to be a winning map generator.
...
def handle_kafka_message(sender_id, data):
    ...
    if msg_type == "wasd-update":
        ...
        game.remote_enemies.pop(sender_id)
...
# Sound Effects
def load_sound_variants(base_path, base_name, variants, extension, volume=1.0):
    """
    Loads a group of sound variants given a base name and range of extensions.
    
    Args:
        base_path (str): Directory containing the sounds.
        base_name (str): The base name of the file (e.g., 'Shot').
        variants (list[str]): Suffixes like ['1', '2', ..., 'A'].
        extension (str): File name extension (e.g., 'mp3', 'wav', etc.)
        volume (float): Volume to set for all loaded sounds (0.0 to 1.0).
    
    Returns:
        list[pygame.mixer.Sound]: List of loaded and volume-adjusted sounds.
    """
    sounds = []
    for variant in variants:
        filename = f"{base_name}{variant}.{extension}"
...
# Load the Sounds
shot_sounds = load_sound_variants(SOUND_FOLDER, "SingleShot", 
                                   [str(i) for i in range(1, 10)] + ['A'],
                                   "mp3", volume=0.03)
local_enemy_hit_sounds = load_sound_variants( \
    SOUND_FOLDER, "OilBarrelHit", [str(i) for i in range(1, 10)] + ['A'],
    "mp3", volume=1.0)
remote_enemy_hit_sounds = load_sound_variants(
    SOUND_FOLDER, "metallic-ting-", [str(i) for i in range(1, 8)],
    "mp3", volume=0.2)
player_hit_sounds = load_sound_variants(SOUND_FOLDER, "PainGrunt",
                                        [str(i) for i in range(1, 6)],
                                        "wav", volume=0.6)
player_death_sound = pygame.mixer.Sound(f"{SOUND_FOLDER}/DeathScream.wav")
player_death_sound.set_volume(0.8)
player_wohoo_sound = pygame.mixer.Sound(f"{SOUND_FOLDER}/Wohoo.mp3")
player_wohoo_sound.set_volume(0.1)
background_music = pygame.mixer.Sound(f"{SOUND_FOLDER}/JungleSkirmishLoop.mp3")
background_music.set_volume(0.2)

# Increase and Reserve Channels
pygame.mixer.set_num_channels(16)         # Make sure we have enough channels
pygame.mixer.set_reserved(2)              # Reserve the first 2 for important sounds
scream_channel = pygame.mixer.Channel(0)  # Reserved for winning/death scream
music_channel = pygame.mixer.Channel(1)   # Reserved for background music
# Start playing the background music
music_channel.play(background_music, loops=-1)

# Reduce volume when going to GAME_WATCHING
def reduce_volume_of_sounds():
    for sound_list in [
        shot_sounds,
        local_enemy_hit_sounds,
        remote_enemy_hit_sounds,
        [background_music]
    ]:
...
# Increase volume when returning from GAME_WATCHING to GAME_PLAYING
def increase_volume_of_sounds():
    for sound_list in [
        shot_sounds,
        local_enemy_hit_sounds,
        remote_enemy_hit_sounds,
