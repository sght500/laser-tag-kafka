...
ENEMY_TIMERS = [150,175,250,275,325,425,450]
    ...
    def move(self, keys, bricks):
        ...
        if self.processing_buffer:
            ...
            if buffer_time >= DG_BUFFER_TIME:
                ...
                send_bullet_created(producer, player_id, (bullet.rect.x+2, bullet.rect.y+2), (bullet.dx, bullet.dy))
...
player_id = ''.join(random.choice(string.ascii_lowercase + string.ascii_uppercase + string.digits) for _ in range(25))

def send_wasd_update(producer, player_id, position, keys):
    message = {
        "type": "wasd-update",
        "position": position,
        "keys": keys,
    ...
    producer.produce("game-events", key=player_id, value=json.dumps(message).encode())

def send_bullet_created(producer, player_id, position, velocity):
    message = {
        "type": "bullet-created",
        "position": position,
        "velocity": velocity,
...
# Warm up Kafka messaging
warmup_message = {
    "type": "warmup",
    "timestamp": time.time()
}

producer.produce("game-events", key=player_id, value=json.dumps(warmup_message).encode())

# Let the producer process the delivery
producer.poll(0)

# Force message to be sent immediately
producer.flush()
...
while running:
    ...
    if keys != prev_keys:
        send_wasd_update(producer, player_id, (player.rect.x, player.rect.y), keys)
        prev_keys = keys
