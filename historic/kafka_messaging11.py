...
KAFKA_MAP_TIMEOUT = 9  # Number of seconds to receive the current map.
KAFKA_LOG_TIMEOUT = 10  # Discard messages which are older than X seconds.
    ...
    def poll_messages(self, handle_function):
        ...
        try:
            ...
            if key == self.player_id:
                print("discarding my own message")
                return  # Skip messages from self

            value = json.loads(msg.value().decode())
            timestamp = value.get("timestamp")
            if time.time() - timestamp > KAFKA_LOG_TIMEOUT:
    ...
    def read_map_and_activity(self):
        ...
        while True:
            ...
            if key in latest_data:
                print ("key found in latest_data")
