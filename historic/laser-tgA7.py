...
# Sound Effects
def load_sound_variants(folder, base_name, extension, start=1, end=1, letters=False):
    sounds = []
    if letters:
        for c in string.ascii_uppercase[:end]:
            path = os.path.join(folder, f"{base_name}{c}.{extension}")
            if os.path.exists(path):
                sounds.append(pygame.mixer.Sound(path))
    else:
        for i in range(start, end + 1):
            path = os.path.join(folder, f"{base_name}{i}.{extension}")
            if os.path.exists(path):
                sounds.append(pygame.mixer.Sound(path))
    return sounds
...
# Shots
shot_sounds = load_sound_variants(SOUND_FOLDER, "SingleShot", "mp3", start=1, end=9) + \
              load_sound_variants(SOUND_FOLDER, "SingleShot", "mp3", end=1, letters=True)
# Hitting enemy
enemy_hit_sounds = load_sound_variants(SOUND_FOLDER, "OilBarrelHit", "mp3", start=1, end=9) + \
                   load_sound_variants(SOUND_FOLDER, "OilBarrelHit", "mp3", end=1, letters=True)
# Hitting player
player_hit_sounds = load_sound_variants(SOUND_FOLDER, "PainGrunt", "wav", start=1, end=5)
# Player dying
player_death_sound = pygame.mixer.Sound(os.path.join(SOUND_FOLDER, "DeathScream.wav"))
