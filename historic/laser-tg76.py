from kafka_messaging26 import KafkaMessenger, KafkaMessageHandler, KafkaMapReader
import string
import pygame
import random
import time
...
MAX_AGE_LAST_ACTIVITY = 90  # Seconds from last activity to create a new maze.
...
map_reader = KafkaMapReader(player_id, bootstrap_servers=BOOTSTRAP_SERVERS)
...
TIME_AFTER_GET_MAP = 1  # Seconds after getting a map to declare "got_a_map".
MAX_TIME_WITHOUT_MAP = 5  # Max seconds to get the map.
...
while running:
    ...
    if game_state == GAME_MENU:
        ...
        if got_a_map:
            ...
            reuse_map = False
            # Try to get the map.
            map_reader.poll_map_or_last_activity_age()            
            if map_reader.map:
                if map_reader.last_activity_age:
                    if map_reader.last_activity_age <= MAX_AGE_LAST_ACTIVITY:
                        reuse_map = True
                    else:
                        print(f"last_activity_age {map_reader.last_activity_age}")
                        reuse_map = False
                else:
                    reuse_map = True
                map_elapsed = time.time() - map_reader.map_time
                if map_elapsed >= TIME_AFTER_GET_MAP:
                    # Time's up. You have to take a decision now.
                    got_a_map = True
                    if reuse_map:
                        print("✅ Reusing maze from Kafka topic.")
                        maze = Maze(map=map_reader.map)
                    else:
                        print("🌀 Creating new maze and publishing to Kafka topic.")
                        maze = Maze(HEIGHT//WALL_PIXELS, WIDTH//WALL_PIXELS, WALL_PIXELS)  # Generate a new one
                        messenger.send_map(maze.map)
                ...
                if menu_elapsed >= MAX_TIME_WITHOUT_MAP:
                    got_a_map = True
                    print("🌀 Creating new maze after waiting max time without map.")
                    maze = Maze(HEIGHT//WALL_PIXELS, WIDTH//WALL_PIXELS, WALL_PIXELS)  # Generate a new one
                    messenger.send_map(maze.map)

        pressed = pygame.key.get_pressed()
        if pressed[pygame.K_F5] and got_a_map:
            game_state = GAME_PLAYING
            print("Setting up the Game")
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                running = False
            elif event.type == player_timer and got_a_map:
                messenger.send_waiting_2_join()
                # print("sent waiting to join")
