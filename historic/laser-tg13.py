...
NO_KEY = -999
MAX_HEALTH = 1400
BULLET_HURT_SPEED = 50
HEALT_RECOVERY_SPEED = 0
...
class Player:
    ...
    def move(self, keys, bricks):
        original_y = self.rect.y  # Store original position
...
class Enemy:
    def __init__(self, x, y):
        self.rect = pygame.Rect(x, y, 20, 20)
        self.hurt_rect = pygame.Rect(x + 18, y + 2, 0, 16)
        self.health = MAX_HEALTH

    def move(self):
        self.rect.x += random.choice([-1, 1]) * SPEED
        self.rect.y += random.choice([-1, 1]) * SPEED
        if self.health < MAX_HEALTH: self.health += HEALT_RECOVERY_SPEED
        self.hurt_rect.update(
            self.rect.x + 2,
            self.rect.y + 2,
            14 * (MAX_HEALTH - self.health) / MAX_HEALTH,
            16
        )
