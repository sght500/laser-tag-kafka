...
class KafkaMapReader:
    def __init__(self, player_id, bootstrap_servers="localhost:9092", topic=f"game-events"):
        self.topic = topic
        self.consumer = Consumer({
            "bootstrap.servers": bootstrap_servers,
    ...
    def read_map_and_last_activity_age(self):
        map, last_activity_age = None, None

        start_time = time.time()
        while True:
            msg = self.consumer.poll(0.06)
            if msg is None:
            ...
            if key == 'map':
                map = value
                print("A map got at {:.3f}s".format(time.time() - start_time))
            else:
                type = value.get("type")
                if type != "warmup":
                    last_activity_age = time.time() - value.get("timestamp")
        if last_activity_age: print(f"Last activity was {last_activity_age:.3f}s ago")
        return map, last_activity_age
